...
export default function AdminDashboard() {
  ...
  // دوال الحذف والتحديث - Moved here for better reliability
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50" dir="rtl">
        <form onSubmit={handleLogin} className="p-8 bg-white shadow-md rounded-lg">
          <h2 className="text-xl font-bold mb-4 text-black">دخول الإدارة</h2>
          <input 
            type="password" 
          ...
          />
          <button type="submit" className="w-full bg-black text-white py-2 rounded hover:bg-gray-800 transition-colors">دخول</button>
        </form>
      </div>
    );
  }
  ...
  // 2. حالة التحميل
  if (loading) return <p className="p-10 text-center" dir="rtl">جاري التحميل...</p>;

  // 3. شاشة الإدارة الرئيسية
  return (
    <div className="p-8 max-w-5xl mx-auto" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-black">إدارة التعليقات</h1>
        <button onClick={() => setIsAuthenticated(false)} className="text-sm text-gray-500 underline">خروج</button>
      </div>
      
      <div className="overflow-x-auto">
        <table className="w-full border-collapse bg-white shadow-sm border text-black">
          <thead>
